...
export async function registrarEstadoVehiculoService(idVehiculo, nuevoKilometraje, reporteFalla) {
    try {
        ...
        if (!vehiculo) {
            return { exito: false, mensaje: "El veh├¡culo no existe en el sistema." };
        }
        ...
        //  Gesti├│n de Fallas y Notificaci├│n
        if (reporteFalla && reporteFalla.gravedad) {
            ...
            if (reporteFalla.gravedad === 'Alta') {
                ...
                alertaEnviada = true;
                console.log(`[NOTIFICACI├ôN] Alerta: El veh├¡culo ${vehiculo.patente} ha sido puesto 'Inactivo' por falla grave.`);
            }
        }
